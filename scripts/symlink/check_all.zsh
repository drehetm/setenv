...
print_section "Check all symlinks"

echo "Legend:"
echo "✔ - Means that the symlink is correct."
echo
...
(
  ...
  while read -r symlink_path destination_path; do
    ...
    check_symlink "$symlink_path" "$destination_path" || check_failed="true"
  done < "$SYMLINK_MAPPING_FILE"

  if str_to_bool $check_failed; then
    exit 1
  fi
) || exit 1
