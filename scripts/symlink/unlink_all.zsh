...
print_section "Unlink all symlinks"

echo "Legend:"
echo "✘ - Means that the symlink doesn't exist."
echo
...
(
  # Track if any unlink operation has failed
  rm_failed="false"
  ...
  while read -r symlink_path destination_path; do
    ...
    remove_symlink "$symlink_path" "$destination_path" || rm_failed="true"
  done < "$SYMLINK_MAPPING_FILE"

  if str_to_bool $rm_failed; then
    exit 1
  fi
) || exit 1
